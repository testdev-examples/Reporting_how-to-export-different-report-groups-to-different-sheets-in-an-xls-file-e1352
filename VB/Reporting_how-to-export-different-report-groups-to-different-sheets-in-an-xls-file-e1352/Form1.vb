...
Namespace WindowsApplication1
	...
	Public Class ExportToXlsCommandHandler
		...
		Public Overridable Sub HandleCommand(ByVal command As PrintingSystemCommand, ByVal args() As Object, ByVal control As IPrintControl, ByRef handled As Boolean)
		...
		Public Overridable Function CanHandleCommand(ByVal command As PrintingSystemCommand, ByVal control As IPrintControl) As Boolean
			' This handler is used for the ExportGraphic command.
			Return command Is PrintingSystemCommand.ExportXls
	...
	Public Class SeparateSheetsExportHelper
		...
		Friend Shared Sub Export(ByVal fileName As String, ByVal xlsExportOptions As XlsExportOptions)
			...
			DirectCast(ObjWorkBookGeneral.Sheets(1), Worksheet).Name = If(ds.Categories(0).Description.Length > 30, ds.Categories(0).Description.Substring(0,30), ds.Categories(0).Description)
			...
			Try
				For i As Integer = 1 To ds.Categories.Count - 1
					...
					ObjWorkSheet = DirectCast(ObjWorkBookTemp.Sheets(1), Excel.Worksheet)
					...
					ObjWorkBookTemp.Close(Excel.XlSaveAction.xlSaveChanges,Type.Missing,Type.Missing)
			...
			Finally
				If Not isObjWorkBookTempClosed Then
					ObjWorkBookTemp.Close(Excel.XlSaveAction.xlSaveChanges,Type.Missing,Type.Missing)
				End If
				ObjWorkBookGeneral.Close(Excel.XlSaveAction.xlSaveChanges,Type.Missing,Type.Missing)
				ObjExcel.Quit()
